...
const getNames = (arr) => {
  ...
  let result = newarr.map(element=> element.split('').reverse().join(''));
  return result;
...
let newArray = [];
  input.forEach(arr=>{
    newArray.push(arr.filter(element => element === target))
  })
  let result = newArray.toString();
  let finalResult= result.split(',').join('').length;
  return finalResult;
...
const totalSum = (input) => {
  ...
  let total =0;
  input.map(item =>{
    item.map(number => total =total +number )
  })
  return total;
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let arr = input.map(element=>{
    return element.filter(number=> typeof number== 'number'&& number %5===0).map(number=>Math.pow(2,number))
   })
   return arr;
